...
def WikiParse(page):
    ...
    regexp_convert_tab = [ (r"'''.*?'''",    lambda instr : '<i>' + instr[3: -3] + '</i>'),
                           (r"\*.*?\*",      lambda instr : '<b>' + instr[1:-1] + '</b>'),
                           (r"={5}.+?={5}",  lambda instr : '<h5>' + instr[5:-5] + '</h5>'),
                           (r"={4}.+?={4}",  lambda instr : '<h4>' + instr[4:-4] + '</h4>'),
                           (r"={3}.+?={3}",  lambda instr : '<h3>' + instr[3:-3] + '</h3>'),
                           (r"={2}.+?={2}",  lambda instr : '<h2>' + instr[2:-2] + '</h2>'),
                           (r"={1}.+?={1}",  lambda instr : '<h1>' + instr[1:-1] + '</h1>'),
                           (r"image\(.+?\)", lambda instr : __getImgSrcHTML(instr) )
                         ]
